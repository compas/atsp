...
*
     ...
     :       0.d-17,1.d-12,1.d-20,1.d0,1.d0/
      ...
      pointer(pnze_t,nze_t(njv)),(pnze_c,nze_c(njv))
...
*     ------------------------------------------------------------------
      ...
      call alcmat(skip,ncfg,nume,nze_c(ind_jj),nze_t(ind_jj),
     :            lhmx_memory,lhmx_disk,jj)
      if (lhmx_memory) nze = nze_t(ind_jj)
      if (idisk == 1) nze = nze_c(ind_jj)
...
!       write(0,'(5F12.9)') (eigvec(i),i=1,nume*N)
     ...
     :                         nloops,nmv
         ...
         write(iscw,'(4F16.12)') (eigvec(ist+i),i=1,nume)
         write(iscw,*) 'Delta Lambda:    '
         write(iscw,'(4f16.12)') (eigvec(ist+nume+i),i=1,nume) 
         write(iscw,*) 'Residuals:       '
         write(iscw,'(4f16.12)')  (eigvec(ist+2*nume+i),i=1,nume)
         WRITE (iounew, '(//A8,I4,2X,A8,I4)' ) '  2*J = ',JJ,'NUMBER =',
     :           req_eigen; 
...
!         call prprty_brci(jj,nume,leigen,eigvec,Ssms)
     ...
     :        eigvec(ncfg*nume+1),EC,shift,iounew,iuc)
      end if
