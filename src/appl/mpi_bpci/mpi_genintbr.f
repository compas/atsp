...
10      n = 0
		...
		if ( gen .and. (mod(n,nprocs) .eq. myid) ) then
		  ...
		  j2 = i2+nclosd
		  j4 = i4+nclosd
		  value(n) = fk(j2,j4,k,rel)
                  if (abs(value(n)).lt.1.e-99) stop
...
*       .. allocate memory for integral book keeping
        if (.not. gen) then
          ...
          call dfill(n,0.D0,value,1)
          call ifill(n,0,ipackn,1)
          gen = .true.
	  go to 10
	end if
* ... allocate tmp space for gdsum
        call alloc(qtmp,4096,8)	
*        print*, 'calling mpi_allr_dp with n=',n, myid
	call mpi_allr_dp(value,n,tmp)
*	print*, 'calling mpi_allr_int'
	call mpi_allr_int(ipackn,n)
*	print*, 'after gdsumi'
	call dalloc(qtmp,4096)
